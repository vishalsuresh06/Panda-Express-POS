...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        employee.save()
        employee2 = Employee.objects.create(name='John America', password='ILoveAmerica', is_manager=False, wage=7.50).save()
        employee3 = Employee.objects.create(name='Chris Panda', password='ILovePandas', is_manager=True, wage=15.00).save()

        # Food Items
        orange = FoodItem.objects.create(name='Orange Chicken', type="Entree", on_menu=True, alt_price=6.00, upcharge=0.00).save()
        beijingBeef=FoodItem.objects.create(name='Beijing Beef', type="Entree", on_menu=True, alt_price=6.00, upcharge=0.00).save()
        broccoliBeef =FoodItem.objects.create(name = 'Broccoli Beef', type="Entree", on_menu = True, alt_price = 6.00, upcharge = 0.00).save()
        eggRoll=FoodItem.objects.create(name = 'Egg Roll', type="Appetizer", on_menu = True, alt_price = 2.00, upcharge = 0.00).save()
        kungPaoChicken=FoodItem.objects.create(name = 'Kung Pao Chicken', type="Entree", on_menu = True, alt_price = 6.00, upcharge = 0.00).save()
        honeyWalnutShrimp=FoodItem.objects.create(name = 'Honey Walnut Shrimp', type="Entree", on_menu = True, alt_price = 6.00, upcharge = 1.50).save()
        friedRice=FoodItem.objects.create(name = 'Fried Rice', type="Side", on_menu = True, alt_price = 0.00, upcharge = 0.00).save()
        chowMein=FoodItem.objects.create(name = 'Chow Mein', type="Side", on_menu = True, alt_price = 0.00, upcharge = 0.00).save()
        teriyakiChicken=FoodItem.objects.create(name = 'Teriyaki Chicken', type="Entree", on_menu = True, alt_price = 6.00, upcharge = 0.00).save()
        broccoliBeef= FoodItem.objects.create(name = 'Broccoli Beef', type="Entree", on_menu = True, alt_price = 6.00, upcharge = 0.00).save()
        FoodItem.objects.create(name = 'Super Greens', type="Side", on_menu = True, alt_price = 0.00, upcharge = 0.00).save()
        FoodItem.objects.create(name = 'Rangoons', type="Appetizer", on_menu = True, alt_price = 2.00, upcharge = 0.00).save()
        FoodItem.objects.create(name = 'Fountain Drink', type="Drink", on_menu = True, alt_price = 0.00, upcharge = 0.00).save()
        ...
        #inventory
        breadedchicken = InventoryItem.objects.create(name='Breaded Chicken', is_food=True, stock=1000, restock_threshold=500, restock_amount = 1000).save()
