...
function DBEditorTable({ title, fetchData, addData, removeData, modifyData }) {
  ...
  return (
    <section>
          ...
          <tr>
            {items.length > 0 &&
              Object.keys(items[0]).map((key, index) => (
                <td className="mngr-font" key={index}>
                  {key}
                </td>
              ))}
            <td></td>
          ...
          {items.map((menuItem, index) => (
            <tr key={index}>
              {Object.values(items[index]).map((entry, index2) => (
                <td className="mngr-font" key={index2}>
                  {entry?.toString() || ""}
                </td>
              ))}
              <td>
                <button
                  onClick={(event) => handleEditSave(index, event.target)}
                >
      ...
      <button className="mngr-createbutton" onClick={() => handleAdd()}>
        Create {title}
      </button>
    </section>
  );
...
function EmployeeEdit() {
  ...
  async function addEmployee(data) {
    const newEmployee = { name: "Enter Name", is_manager: false, wage: 7.0 };
...
function MenuEdit() {
  ...
  return (
    <DBEditorTable
      ...
      fetchData={fetchMenu}
      addData={addMenu}
      removeData={removeMenu}
      modifyData={modifyMenu}
    />
  );
}
...
function InventoryEdit() {
  ...
  return (
    <DBEditorTable
      ...
      removeData={removeInventory}
      modifyData={modifyInventory}
    />
  );
}
...
function SettingsInput({ name, desc, field, type }) {
  ...
  if (type == "text") {
    inputComponent = (
      <input type="text" value={settings[field]} onChange={changeSettings} />
    );
  } else if (type == "checkbox") {
    inputComponent = (
      <input
...
function KitchenSettings() {
  ...
  return (
    <KitchenSettingsContext.Provider value={{ settings, setSettings }}>
      <div className="mngr-kitchensettings">
        ...
        <table className="mngr-settingsTable">
          ...
          <tbody>
            ...
            <SettingsInput
              name="Pending Order Color"
              field="kt_pendingColor"
              type="color"
            />

            <SettingsInput
              name="In Progress Order Color"
              field="kt_inprogressColor"
              type="color"
            />

            <SettingsInput
              name="Completed Order Color"
              field="kt_completedColor"
              type="color"
            />

            <SettingsInput
              name="Canceled Order Color"
...
function Excess() {
  ...
  const queryExcess = async () => {
    ...
    try {
      let response = await fetch(
        `${apiURL}/api/manager/excess?timestamp=${targetTime}`
      );
  ...
  return (
    <div className="mngr-excess mngr-font">
...
function SellsTogether() {
  ...
  const queryPairs = async () => {
    ...
    try {
      let response = await fetch(
        `${apiURL}/api/manager/sellstogether?startDate=${startDate}&endDate=${endDate}`
      );
  ...
  return (
    <div className="mngr-sellstogether mngr-font">
...
function Restock() {
  ...
  return (
    <div className="mngr-restockReport mngr-font">
        ...
        <tbody>
          {restockList.map((invItem, index) => (
            <tr key={index}>
              ...
              <td>
                <button onClick={restockItem(invItem)}>
                  Restock (Purchase {invItem.restock_amount})
                </button>
              </td>
            </tr>
          ))}
...
function Sales() {
  ...
  const fetchData = async () => {
    ...
    const promises = selectedIds.map(async (id) => {
      try {
        let response = await fetch(
          `${apiURL}/api/manager/menu-query/${id}?start_date=${startDate}&end_date=${endDate}`
        );
  ...
  return (
    <div className="mngr-salescontainer">
      <div className="mngr-salescol">
        {chartData ? <Line data={chartData} /> : <p>Send a query</p>}
      </div>
      <div className="mngr-salescol">
        <div>
          <h3>Select Items</h3>
...
function ProductUsage() {
  ...
  const fetchData = async () => {
    ...
    const promises = selectedIds.map(async (id) => {
      try {
        let response = await fetch(
          `${apiURL}/api/manager/inventory-query/${id}?start_date=${startDate}&end_date=${endDate}`
        );
...
function XReport() {
  ...
  return (
    <div className="mngr-xreport mngr-font">
      <h2>X Report</h2>
...
function ZReport() {
  ...
  return (
    <div className="mngr-zreport mngr-font">
      <h2>Z Report</h2>
      <button onClick={fetchData}>Refresh</button>
      <br></br>
...
function OrderItemCard({ orderItem }) {
  return (
    <div className="mngr-orderItemCard">
...
function OrderCard({ order, handleDelete }) {
  return (
    <div className="mngr-orderCard">
      <div className="mngr-orderCardHeaders">
        <h3 className="mngr-orderInfo">
          {" "}
      ...
      <div className="mngr-orderCardBody">
        ...
        <div className="mngr-cardButtons">
          <button className="kt-cancel" onClick={() => handleDelete(order)}>
            {" "}
            Delete{" "}
          </button>
        </div>
...
function OrderHistory() {
  ...
  const fetchOrders = async () => {
    try {
      let response = await fetch(
        `${apiURL}/api/manager/orderhistory?date=${date}`
      );
  ...
  return (
    <div className="mngr-orderhist mngr-font">
      <h2>Order History</h2>
    ...
    <>
      ...
      <div>
        ...
        <div className="mngr-nav">
          <Link to="/cashier" className="mngr-btn mngr-btn-ret">
            Cashier
          </Link>
          <Link to="/manager/employees" className="mngr-btn">
            Employees
          </Link>
          <Link to="/manager/menu" className="mngr-btn">
            Menu
          </Link>
          <Link to="/manager/inventory" className="mngr-btn">
            Inventory
          </Link>
          <Link to="/manager/sales" className="mngr-btn">
            Sales
          </Link>
          <Link to="/manager/productusage" className="mngr-btn">
            Product Usage
          </Link>
          <Link to="/manager/kitchensettings" className="mngr-btn">
            Kitchen Settings
          </Link>
        </div>
        <div className="mngr-nav">
          <Link to="/manager/excess" className="mngr-btn">
            Excess Inventory
          </Link>
          <Link to="/manager/sellstogether" className="mngr-btn">
            What Sells Together
          </Link>
          <Link to="/manager/restock" className="mngr-btn">
            Restock Report
          </Link>
          <Link to="/manager/xreport" className="mngr-btn">
            X Report
          </Link>
          <Link to="/manager/zreport" className="mngr-btn">
            Z Reports
          </Link>
          <Link to="/manager/orderhistory" className="mngr-btn">
            Order History
          </Link>
        </div>
        <Outlet />
