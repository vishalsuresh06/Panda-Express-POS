...
export default function Customers() {
  ...
  const remove_item = (id_to_remove) => {
    setItems((items) => items.filter((item) => item.array_id !== id_to_remove));
  };
    ...
    if (sysState === "Bowl") {
      ...
      return (
        <BuildFood
          ...
          typeID={sysState}
          typePrice={
            orderTypes.find((item) => item.name === sysState).base_price
          }
        />
      );
    }
...
function OrderButtons({ setSys, orderTypes }) {
  ...
  return (
    <ul className="CK-subMenuOptions">
      {orderTypes
        .filter((type) => !removeID.includes(type.name))
        .map((type) => (
...
function FoodCard({ id, menu, setOrd, ord, max }) {
  ...
  function buttonHandle() {
    ...
    if (
      clicked === 0 &&
      ord.filter((item) => item.type === currItem.type).length < max
    ) {
      setOrd((current) => [
        ...current,
        {
          array_id: numExtra,
          alt_price: currItem.alt_price,
          id: currItem.id,
          name: currItem.name,
          upcharge: currItem.upcharge,
          type: currItem.type,
        },
      ]);
      setclick(1);
  ...
  function killAnother() {
    setOrd((current) =>
      current.filter(
        (item) => !(item.id === currItem.id && item.array_id === numExtra)
      )
    );
    setExtra(numExtra - 1);
  }
  function MoreButton() {
    if (
      ...
      return (
        <div>
          <button className="CK-addAnother" onClick={addAnother}>
            add another
          </button>
        </div>
      );
  ...
  function LessButton() {
    if (numExtra > 0) {
      return (
        <div>
          <button className="CK-removeOne" onClick={killAnother}>
            remove 1
          </button>
        </div>
      );
...
function BuildFood({
  ...
  function CompleteButton() {
    if (
      numEntree === currOrder.filter((item) => item.type === "entree").length &&
      numSide === currOrder.filter((item) => item.type === "side").length
    ) {
      return (
      ...
      return <div className="CK-nonCompleteButton">Order incomplete</div>;
    }
  }

  return (
    <div className="CK-BuildFoodPage">
      <button onClick={() => setSys(-1)} className="CK-cancelOrder">
        Back
      </button>
      <h3>Entree</h3>
      <h5>
        Pick {numEntree} entree{Es()}
      </h5>
...
function SinglePick({ orderType, typeID, menu, addItem, setSys }) {
  ...
  function completeOrder() {
    ...
    currOrder.map((item) => addItem(typeID, item.alt_price, [item]));
    setSys("");
  }
  return (
    <div className="CK-singlePick">
      ...
      <ul className="CK-singlePickList CK-sides">
        {menu
          .filter((item) => item.type === orderType.tolowercase())
          .map((item) => (
            <li key={item.id}>
              <FoodCard
                id={item.id}
